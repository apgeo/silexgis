Q3D.gui = {
  ...
  addModelParams: function () {

	var parameters = this.parameters;
  	var model_folder = this.gui.addFolder('Cave');
	
    // Plane height
    model_folder.add(parameters.model, 'vertical_exaggeration').min(0).max(50).name('Vertical exaggeration').onChange(function (value) {
      //if (customPlane === undefined) addPlane(parameters.cp.c);
      //customPlane.position.z = (value + project.zShift) * project.zScale;
