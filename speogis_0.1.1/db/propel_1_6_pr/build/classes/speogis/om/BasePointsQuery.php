 ...
 * @method PointsQuery orderByGeom($order = Criteria::ASC) Order by the geom column
 ...
 * @method PointsQuery groupByGeom() Group by the geom column
 ...
 * @method Points findOneByGeom(string $geom) Return the first Points filtered by the geom column
 ...
 * @method array findByGeom(string $geom) Return Points objects filtered by the geom column
...
abstract class BasePointsQuery extends ModelCriteria
{
    ...
    {
        $sql = 'SELECT `id`, `lat`, `long`, `elevation`, `coords`, `gpx_name`, `gpx_sym`, `gpx_type`, `gpx_cmt`, `gpx_sat`, `gpx_fix`, `gpx_time`, `_type`, `_details`, `added_by_user_id`, `add_time`, `_id_point_type`, `geom`, `spatial_geometry` FROM `points` WHERE `id` = :p0';
        try {
    ...
    /**
     * Filter the query on the geom column
     *
     * Example usage:
     * <code>
     * $query->filterByGeom('fooValue');   // WHERE geom = 'fooValue'
     * $query->filterByGeom('%fooValue%'); // WHERE geom LIKE '%fooValue%'
     * </code>
     *
     * @param     string $geom The value to use as filter.
     *              Accepts wildcards (* and % trigger a LIKE)
     * @param     string $comparison Operator to use for the column comparison, defaults to Criteria::EQUAL
     *
     * @return PointsQuery The current query, for fluid interface
     */
    public function filterByGeom($geom = null, $comparison = null)
    {
        if (null === $comparison) {
            if (is_array($geom)) {
                $comparison = Criteria::IN;
            } elseif (preg_match('/[\%\*]/', $geom)) {
                $geom = str_replace('*', '%', $geom);
                $comparison = Criteria::LIKE;
            }
        }

        return $this->addUsingAlias(PointsPeer::GEOM, $geom, $comparison);
    }
