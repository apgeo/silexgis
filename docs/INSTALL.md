...
| `SILEXGIS__TripCallout__ReminderLead` | `2.00:00:00` | how far ahead of a trip the people on it are reminded that it is coming up. The same pass sends it, so a trip put back or called off stops reminding anybody. `00:00:00` sends no reminders and leaves the overdue check running |
