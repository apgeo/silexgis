...
public sealed class TripCalloutSweepTests : IAsyncLifetime, IDisposable, IClassFixture<PostgresFixture>
{
    ...
    private Task<List<Notification>> AlarmsAboutAsync(Guid userId, Guid tripId) =>
    ...
    /// </summary>
    private async Task<List<Notification>> NoticesAboutAsync(
        Guid userId, string templateKey, Guid tripId)
    {
        using var scope = factory.Services.CreateScope();
        var db = scope.ServiceProvider.GetRequiredService<SilexGisDbContext>();
        var sent = await db.Notifications.AsNoTracking()
            .Where(x => x.RecipientUserId == userId && x.TemplateKey == templateKey)
            .OrderBy(x => x.Id)
            .ToListAsync();
