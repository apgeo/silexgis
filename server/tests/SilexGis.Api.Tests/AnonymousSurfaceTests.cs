...
public sealed class AnonymousSurfaceTests(PostgresFixture postgres) : IDisposable, IClassFixture<PostgresFixture>
{
    ...
    [
        ...
        ("GET", "/api/v1/public/trips/{token}/past", "the same link asked what else this one cave has been followed on: trips of it that were published and are now over, by title, dates, a headcount and a \"there is something to play\" bit — naming nobody and carrying no position. A trip nobody published, one whose links were all revoked, and one still being followed are all absent. The link opens this on a second, longer window than the live page's, deliberately, so that a club's history stays readable after tonight's party is out; outside both windows it answers exactly what an invented token answers, so nothing here says a token was once real"),
        ("GET", "/api/v1/public/trips/{token}/past/{tripLogId:guid}", "one of those past trips played back — the party by their place in it, their teams, and where each was reported over time, drawn on the survey that trip's own reports were measured in. The cave's protection refusal, the per-participant caption and the per-row anchor are all re-decided on every read; the trip must belong to this link's cave and must no longer be live; and every refusal is the one 404 an unknown token gets"),
        ("GET", "/api/v1/public/trips/{token}/live", "the same link asked who is in this one cave right now: every trip of it being followed at this moment, each with its party — the answer the archive route beside it deliberately refuses, because a share token names one trip and a club running two parties into one cave could show neither beside the other. It names a trip, its dates, its watch's state and the party as the followed page names them, and carries no cave, survey or caver identity. Each party is placed against THIS link's survey rather than its own trip's, so a report measured on a different one comes back placeless and says so — a reader is never handed a station from one survey to draw on another. It opens on the same combined window the archive does, which is what lets a page whose own trip has closed still show the parties still underground; the consequence, that an old link reports current parties for as long as its trip stays readable as past, is bounded by the archive's retention setting. Every refusal is the one 404 an invented token gets"),
