...
public static class CavingGroupPermissionSeeder
{
    ...
    /// <summary>
    /// What the starter ruleset grants over a domain. Two of them differ in one bit —
    /// documents and checklists — because they carry no position, so the right to see an
    /// exact location says nothing about them and granting it would be noise in a ruleset an
    /// operator has to read and edit. The rest hold rows with a geometry of their own — an
    /// expedition's working area as much as a trip's — and so keep the bit. It decides
    ...
    private static AccessAction StarterActions(AccessDomain domain) =>
        ...
        | (domain is AccessDomain.Documents or AccessDomain.Checklists
            ? AccessAction.None
