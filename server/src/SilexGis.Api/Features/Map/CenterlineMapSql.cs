...
/// </para>
/// <para>
/// Which geometry a row gets is decided per row: the stored skeleton at overview zooms, the
/// bbox-clipped survey geometry once the viewport is small enough. Two guards run on stored
/// counts alone, so an oversized centerline never gets fetched out of TOAST storage to find out
/// how big it is: the low-zoom gate, and a running per-request budget.
/// </para>
/// </summary>
public static class CenterlineMapSql
{
    /// <summary>
    /// Caves with a centerline whose bounding box meets the viewport, visibility-filtered. The
    /// <c>&amp;&amp;</c> operator answers from the GIST index alone, so this never pulls a
    /// multi-megabyte geometry out of TOAST storage — it exists so the location-protection rule
    /// can be evaluated (in Domain, where it lives) before any geometry is produced.
    /// </summary>
    public static async Task<IReadOnlyList<Guid>> CaveIdsInViewAsync(
        SilexGisDbContext db, UserContext user, Bbox box, CancellationToken ct)
    {
        var (visibilitySql, parameters) = PermissionSql.VisibleToFragment(user, "c", AttachedEntityType.Cave);
        ...
        var sql = $"""
            SELECT DISTINCT cl.cave_id
            FROM cave_centerlines cl
            JOIN caves c ON c.id = cl.cave_id
            WHERE c.deleted_at IS NULL
              AND cl.geom && ST_MakeEnvelope(@west, @south, @east, @north, 4326)
              AND {visibilitySql}
            """;
    ...
    public static async Task<IReadOnlyList<CenterlineMapRow>> QueryAsync(
        ...
        IReadOnlyCollection<Guid> withheldCaveIds,
        CancellationToken ct)
    {
        ...
        // Location-protected caves are excluded here rather than filtered afterwards, so their
        // geometry is never even produced. They are also not counted anywhere in the response:
        // a centerline traces the cave's exact position, so its existence is not disclosed.
        parameters.Add("protected_cave_ids", PermissionSql.UuidArray([.. withheldCaveIds]));
        ...
        var sql = $"""
            WITH src AS MATERIALIZED (
                SELECT cl.id, cl.cave_id, cl.name, cl.length_m,
                       (@gate_active AND COALESCE(cl.skeleton_path_count, cl.path_count) > @gate_paths) AS gated,
                       CASE
                           WHEN @gate_active AND COALESCE(cl.skeleton_path_count, cl.path_count) > @gate_paths
                               THEN NULL
                           WHEN @detail
                               THEN ST_ClipByBox2D(
                                    ST_Force2D(cl.geom),
                       ...
                       CASE
                           WHEN @gate_active AND COALESCE(cl.skeleton_path_count, cl.path_count) > @gate_paths
                               THEN NULL
                           ELSE COALESCE(cl.skeleton, ST_Force2D(cl.geom))
                       END AS overview_g
                FROM cave_centerlines cl
                JOIN caves c ON c.id = cl.cave_id
                WHERE c.deleted_at IS NULL
                  AND cl.geom && ST_MakeEnvelope(@west, @south, @east, @north, 4326)
                  AND NOT (cl.cave_id = ANY(@protected_cave_ids))
                  AND {visibilitySql}
            ),
