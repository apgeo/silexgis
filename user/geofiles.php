...
?>
<b><h3>*{geofiles.page_title}*</h3></b>
<a class="btn btn-primary" href="<?=WEBROOT ?>/user/add_geofile.php" >*{geofiles.add_geofile}*</a>
<a class="btn btn-primary" href="<?=WEBROOT ?>/user/add_multiple_geofiles.php" >*{geofiles.add_multiple_geofiles}*</a>
<?php
################################################################################   
## +---------------------------------------------------------------------------+
## | 1. Creating & Calling:                                                    | 
...
/*  $sql="SELECT files.id, file_name, user_id, files.add_time, files.file_type as file_type, size, md5_hash, geoobjects_to_files.id, t, username
 ...
 ";*/
  $sql="SELECT geofiles.id, file_name, id_user, geofiles.add_time, geofiles.type as file_type, size, md5_hash, username, geofiles.enabled, extract_style FROM geofiles
	INNER JOIN users ON users.id = geofiles.id_user";
...
##  *** set data source with needed options
  $default_order_field = "id";//"geofiles.id";
...
##  *** set columns in view mode
  $dgrid->setAutoColumnsInViewMode(false);  
    ...
    "file_name" => array("header"=>"*{geofiles.col_name}*", "type"=>"label", "align"=>"left", "width"=>"20px", "wrap"=>"nowrap", "text_length"=>"-1", "case"=>"normal", "readonly"=>true),
    "add_time"  => array("header"=>"*{geofiles.col_time}*", "type"=>"label", "align"=>"left", "width"=>"20px", "wrap"=>"nowrap", "text_length"=>"-1", "case"=>"normal", "readonly"=>true),
    "file_type" => array("header"=>"*{geofiles.col_type}*", "type"=>"label", "align"=>"left", "width"=>"20px", "wrap"=>"nowrap", "text_length"=>"-1", "case"=>"normal", "readonly"=>false),
	"size" => array("header"=>"*{geofiles.col_size}*", "type"=>"label", "align"=>"left", "width"=>"20px", "wrap"=>"nowrap", "text_length"=>"-1", "case"=>"normal", "readonly"=>true),
    "username"  => array("header"=>"*{geofiles.col_user}*", "type"=>"label", "align"=>"left", "width"=>"20px", "wrap"=>"nowrap", "text_length"=>"-1", "case"=>"normal", "readonly"=>false),
	"enabled" => array("header"=>"*{geofiles.col_enabled}*", "type"=>"label", "align"=>"left", "width"=>"20px", "wrap"=>"nowrap", "text_length"=>"-1", "case"=>"normal", "readonly"=>false),
    "extract_style"  => array("header"=>"*{geofiles.col_original_style}*", "type"=>"label", "align"=>"left", "width"=>"20px", "wrap"=>"nowrap", "text_length"=>"-1", "case"=>"normal", "readonly"=>false)
  );
...
##  ***  set settings for edit/details mode
  ...
  //$condition = "files.id = ".$dgrid->f_rid." ";  
    ...
    // $fill_from_array = array("0"=>"No", "1"=>"Yes", "2"=>"Don't know", "3"=>"My be"); /* as "value"=>"option" */
     ...
     $em_columns = array(
		...
		// "long"  =>array("header"=>"Lon",    "type"=>"textbox",  "width"=>"160px", "req_type"=>"rf", "title"=>"Lon", "readonly"=>!false /*"readonly"=>true*/),      	 
		 ...
		 //"file_name"  =>array("header"=>"Name",    "type"=>"textbox",  "width"=>"160px", "req_type"=>"rt", "title"=>"Name", "readonly"=>false, "view_type"=>"textbox"),      
		 //"add_time"  =>array("header"=>"Time",    "type"=>"textbox",  "width"=>"160px", "req_type"=>"rt", "title"=>"", "readonly"=>false, "view_type"=>"textbox"),      
		 "enabled"  =>array("header"=>"Enabled",    "type"=>"checkbox",  "width"=>"160px", "req_type"=>"ri", "title"=>"Enabled", "readonly"=>false, "true_value"=>1, "false_value"=>0,   /*"view_type"=>""*/),
		 "extract_style"  =>array("header"=>"Original style",    "type"=>"checkbox",  "width"=>"160px", "req_type"=>"ri", "title"=>"Original style", "readonly"=>false, "true_value"=>1, "false_value"=>0),
	...
	//$dgrid->setAutoColumnsInEditMode(!false); // $dgrid->setAutoColumnsInEditMode(true);
	 ...
	 //-- to be able to edit something add in edit mode must be 1 ??
	 //-- like this: "add"     =>array("view"=>0, "edit"=>1, "type"=>"link"),
	    $modes = array(
