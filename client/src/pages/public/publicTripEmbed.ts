 ...
 *
 * <b>It stays at 1 while the vocabulary below grows, and that is the versioning rule working
 * rather than being dodged.</b> Both halves compare `v` for equality, so raising it would make
 * every snippet already pasted into every club's website stop talking to the page it frames: no
 * greeting, no party announcement, and every station link in every article dead — an upgrade of
 * this application silently breaking websites this project does not control. What versioning is
 * for is the case where the two halves would <em>misunderstand</em> each other, and that case does
 * not arise here, because the deployment is one-directional. The snippet is generated by the
 * installation it points at, so a relay is never newer than the page; the only pairing that occurs
 * is an old relay with a new page, and an old relay simply never utters the new words — its own
 * list of attributes does not contain them, so a link naming one is left to behave as an ordinary
 * link. That is the protocol's existing discipline for an unknown target, applied to an unknown
 ...
 *
 * The first three name a place in the drawing on screen. The last three name what the drawing
 * <em>is</em>: which trip of this cave is being played, whom the replay keeps up with, and where
 * its clock stands. `trip` takes a past trip's identifier, or the word `live` for the trip this
 * link was published for — which is how an article writes its own way back out of the past.
 */
export type EmbedFocusKind = 'station' | 'survey' | 'caver' | 'team' | 'trip' | 'moment';

/** The value of a `trip` target meaning "the trip this link was published for, as it stands now". */
export const EMBED_TRIP_LIVE = 'live';
...
export interface EmbedFocusMessage {
  ...
  /**
   * Which trip the target is to be read in — a past trip's identifier, or `live`.
   *
   * <b>Carried beside the target rather than folded into it, because one press has to do both.</b>
   * "The survey team at the sump, on the 2019 trip" is one sentence in an article and must be one
   * link: opening the trip and then finding the team are not two things a reader should have to do
   * in order. Absent means whatever the frame is already showing, so a link that only names a team
   * moves the camera within the trip on screen.
   */
  trip?: string;
  /**
   * Where to stand the replay's clock, as an ISO-8601 instant.
   *
   * Optional for the same reason `trip` is: an article can name a moment, a trip, both, or neither.
   * A moment outside the trip's own stretch is brought into it rather than refused — a club writing
...
export interface EmbedReadyMessage {
  ...
  /**
   * Which trip the party above belongs to, and — where it is a past one — where its clock stands.
   *
   * <b>Announced because without it the party above is ambiguous, and dangerously so.</b> The same
   * shape carries the people underground right now and the people who were underground on a trip in
   * 2019, and an article that prints "Ana is at the sump" from a message it believed was live would
   * be saying something false about somebody's whereabouts. So every announcement says which of the
   * two it is, and a host page can print its own words for the past exactly as this page prints
   * its banner.
   *
   * Absent means the trip this link was published for, as it stands now — which is what every
   * announcement meant before the archive existed, so an article written against the older
   * vocabulary keeps reading correctly.
   */
  past?: {
    tripLogId: string;
    title: string;
...
export function parseEmbedInbound(data: unknown): EmbedInbound | null {
  ...
  if (!(EMBED_FOCUS_KINDS as readonly unknown[]).includes(kind)) {
    return null;
  }
  if (typeof ref !== 'string' || ref.length === 0 || ref.length > 400) {
    return null;
  }
  // The two optional members, read as loosely as they arrive and length-capped for the reason the
  // reference is: nothing downstream has any use for a megabyte of trip identifier, and a value of
  // the wrong shape is an absent value rather than a reason to throw.
  return {
    type: 'focus',
...
const RELAY_SOURCE = `(function () {
  ...
  /* The primary target, most specific first: a link naming both a trip and a caver is *about* the
     caver, and the trip is the context it is read in. Written out rather than imported because this
     runs as pasted text in somebody else's page; it is kept in step with EMBED_FOCUS_KINDS by the
     test that reads both. */
  var KINDS = ['station', 'survey', 'caver', 'team', 'moment', 'trip'];
  ...
  /* Built from the same list, so a kind added above cannot be one the click handler never notices. */
  var SELECTOR = KINDS.map(function (kind) {
    return '[data-silexgis-' + kind + ']';
  }).join(',');
  ...
  document.addEventListener('click', function (event) {
    ...
    var link = from.closest(SELECTOR);
    if (!link) return;
    ...
    /* The context a target is read in, carried whatever the primary target was: "the survey team
       at the sump on the 2019 trip" is one sentence and has to be one press. */
    var trip = link.getAttribute('data-silexgis-trip');
