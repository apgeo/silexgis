...
class GPX extends GeoAdapter
{
  ...
  protected function parseWaypoints() {
    ...
    foreach ($wpt_elements as $wpt) {
	  ...
	  //echo "<br/><br/>";echo "<br/><br/> end ";
	  
	  $eo = $this->childElements($wpt, 'ele');
	  $elevation = $eo[0]->nodeValue;
	  	  
      $point = new Point($lon, $lat, $elevation); //-- double elevation assignment here and next in foreach
	  $points[] = $point;
	  
	  foreach ($wpt->childNodes as $child)
	    {
			if ($child->nodeName == 'ele')
				$point->ele = $child->nodeValue;
			else
			if ($child->nodeName == 'time')
				$point->time = $child->nodeValue;
			else
			if ($child->nodeName == 'cmt')
				$point->cmt = $child->nodeValue;
			else
			if ($child->nodeName == 'type')
				$point->type = $child->nodeValue;
			else
			if ($child->nodeName == 'name')
				$point->name = $child->nodeValue;
			else
			if ($child->nodeName == 'sym')
				$point->sym = $child->nodeValue;
			else
			if ($child->nodeName == 'magvar')
				$point->magvar = $child->nodeValue;
			else
			if ($child->nodeName == 'geoidheight')
				$point->geoidheight = $child->nodeValue;
			else
			if ($child->nodeName == 'desc')
				$point->desc = $child->nodeValue;
			else
			if ($child->nodeName == 'src')
				$point->src = $child->nodeValue;
			else
			if ($child->nodeName == 'link')
				$point->link = $child->nodeValue;
			else
			if ($child->nodeName == 'fix')
				$point->fix = $child->nodeValue;
			else
			if ($child->nodeName == 'sat')
				$point->sat = $child->nodeValue;
			else
			if ($child->nodeName == 'hdop')
				$point->hdop = $child->nodeValue;
			else
			if ($child->nodeName == 'vdop')
				$point->vdop = $child->nodeValue;
			else
			if ($child->nodeName == 'pdop')
				$point->pdop = $child->nodeValue;
			else
			if ($child->nodeName == 'ageofdgpsdata')
				$point->ageofdgpsdata = $child->nodeValue;
			else
			if ($child->nodeName == 'dgpsid')
				$point->dgpsid = $child->nodeValue;			
		}
    }
    return $points;
  }
